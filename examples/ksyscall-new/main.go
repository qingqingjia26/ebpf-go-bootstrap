...
//go:generate go run github.com/cilium/ebpf/cmd/bpf2go bpf ksyscall-new.bpf.c -- -I../../src/headers
...
func main() {
	...
	defer tp.Close()
	tp2, err := link.Tracepoint("syscalls", "sys_enter_tgkill", objs.TgkillEntry, nil)
	if err != nil {
		log.Fatalf("failed to link tracepoint: %v", err)
	}
	defer tp2.Close()
